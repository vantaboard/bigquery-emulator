---
name: googlesqlite-01-ddl-catalog
overview: Fix unqualified DDL table names, dataset bootstrap, and catalog 404s blocking 14 googlesqlite tests.
todos:
  - id: gsql-01-ddl-catalog
    content: Qualify DDL names and auto-create datasets for googlesqlite port tests
    status: pending
isProject: false
---

# googlesqlite 01: DDL / Catalog / Gateway SQL

## Goal

Fix unqualified DDL table names, dataset bootstrap, and catalog 404s blocking 14 googlesqlite tests.

Baseline: `20260603T035812Z` (12 failing tests in this bucket).

## Root cause

- `googlesqlite_query_test.go:1945: CREATE TABLE: jobs.query -> 404: {"error":{"code":404,"message":"Not found: Dataset gsql-InformationSchemaColumns:colsds","errors":[{"reason":"notFound","message":"...`
- `googlesqlite_query_test.go:2108: CREATE TABLE: jobs.query -> 404: {"error":{"code":404,"message":"Not found: Dataset gsql-InformationSchemaColumnsA:infoarr","errors":[{"reason":"notFound","message"...`
- `googlesqlite_query_test.go:2054: CREATE TABLE: jobs.query -> 404: {"error":{"code":404,"message":"Not found: Dataset gsql-InformationSchemaColumnsO:op_ds","errors":[{"reason":"notFound","message":"...`
...
3. Auto-create datasets referenced by googlesqlite tests before DDL runs (404 `Not found: Dataset …`).
...
- `./gateway/e2e/testresults/run_googlesqlite_emulator_tests.sh` fail count drops by ~12.
