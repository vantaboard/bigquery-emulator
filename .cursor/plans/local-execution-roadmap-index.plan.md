...
overview: "Route vocabulary and foundation plans; active query port conformance work lives in local-exec-00-index.plan.md"
todos:
  - id: roadmap-local-exec-index
    content: "Execute plans in local-exec-00-index.plan.md (01→16) for the ported query suite"
    status: pending
isProject: false
---
...
**Active conformance work** for the ported query port suite is
...
- [`local-exec-subagent-dispatch.plan.md`](local-exec-subagent-dispatch.plan.md) —
...
Execute before or in parallel with query port plans when registry/router
gaps block progress:
...
Supporting plans (not part of the query port failure sweep):
...
- Every `SHAPE_TRACKER.md` row has a landed route or an explicit
  `(planned)` row with a query port plan owner.
- Every `functions.yaml` row has a non-`kFallback` disposition.
- query port suite: `fail=0` (except intentional skips) per
  [`local-exec-00-index.plan.md`](local-exec-00-index.plan.md).

## Regenerate query port plan test lists
