---
name: googlesqlite-06-aggregate-modifiers
overview: Support BigQuery `array_agg` / `string_agg` modifiers blocking ~8 googlesqlite tests.
todos:
...
# googlesqlite 06: Aggregate Modifiers (ORDER BY / LIMIT / HAVING)

## Goal

Support BigQuery `array_agg` / `string_agg` modifiers blocking ~8 googlesqlite tests.
...
- `googlesqlite_query_test.go:1781: Scan: sql: no rows in result set`
...
- `./gateway/e2e/testresults/run_googlesqlite_emulator_tests.sh` fail count drops by ~8.
