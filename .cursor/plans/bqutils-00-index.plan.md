...
| fixtests-13 bqutils tail | done | 102→110 | trig math (acos/sin/cos/sqrt/atan2), ANY_VALUE + STDDEV aggregates, BYTES/DATETIME casts + CAST FORMAT, DATE_TRUNC/DATE_DIFF on DATETIME, INTEGER→INT64 preprocess; +8 promoted (`azimuth_to_geog_point`, `cw_xml_element_with_attributes`, `linear_regression`, `round_datetime`, vertica upperb/lowerb/substrb, `convert_bytes_string`); **104/110** runner pass (6 legacy fails unchanged); **7** known_failing (incl. deferred `cw_xml_extract` python UDF) |
