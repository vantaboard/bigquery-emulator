# BigQuery Node samples and the bigquery-emulator
...
| **`GOLANG_SAMPLES_PROJECT_ID`** | Same as **`.envrc`**: used when the Google project env vars are unset. |
...
id env vars)—as in this repo’s **`.envrc`** or **GitHub Actions**—requests go to the
bigquery-emulator gateway and **fake-gcs-server** where the samples and client support it.
...
**CI:** In **`.github/workflows/thirdparty-samples.yml`**, the **nodejs-bigquery**
job builds **`gateway_main`**, starts **Docker Compose `fake-gcs-server`**, exports the same
