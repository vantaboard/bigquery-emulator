...
describe('authUserFlow()', () => {
  ...
  before(async () => {
    ...
    const BigQueryStub = sinon.stub().callsFake(() => ({
      createQueryJob: sinon.stub().resolves([
        {
          id: 'auth-flow-query-job',
          getQueryResults: sinon.stub().resolves([[{name: 'William'}]]),
        },
      ]),
    }));
  ...
  it('should run a query', async () => {
    sinon.stub(authUserFlow.main, 'authFlow').resolves({
