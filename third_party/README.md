...
| **CREATE MODEL / ML** + **routine DDL** | Skipped via `bqtestutil.SkipEmulatorBQML()` in Go model/export samples (mirrors Python `emulator_pytest_skip.py` / Node `models.test.js` skip). `CREATE MODEL` and `ML.PREDICT` / `ML.FORECAST` / `ML.EVALUATE` run locally as **stubs** (schema-correct NULL placeholders); third-party samples stay skipped when they need model metadata APIs or non-null prediction values. |
...
### Feature-expansion closeout (2026-06)

Skip-audit sweep after the 2026-06 feature-expansion work (external data
sources, ML/DP stubs, Python UDFs, sequences/catalog refs, measures, and
the scalar/statement long tail) + bq alignment removals (`936b428`,
`6fc21de`):
...
| **dbt `upload_file`** | Still skipped — engine DDL gaps (`TestSimpleMaterializationsBigQuery` triage); not in scope for the external-data-source work. |
