...
// Package testutil provides test helpers for vendored BigQuery golang samples tests.
package testutil
...
func testContext() (Context, error) {
	...
	rootName := "golang-bigquery-tests"
	if !strings.Contains(dir, rootName) {
		return tc, fmt.Errorf("could not find %s directory", rootName)
	}
	tc.Dir = dir[:strings.Index(dir, rootName)+len(rootName)]
