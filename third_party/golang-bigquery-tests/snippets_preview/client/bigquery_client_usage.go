...
func basicClientUsage(ctx context.Context, w io.Writer, useGRPC bool) error {
	...
	if useGRPC {
		grpcOpts := append([]option.ClientOption{}, opts...)
		if os.Getenv("BIGQUERY_EMULATOR_HOST") != "" {
			v2 := bqopts.BigQueryV2GRPCClientOptions()
			if len(v2) == 0 {
				return fmt.Errorf("NewClient: set BIGQUERY_V2_GRPC_ENDPOINT or BIGQUERY_STORAGE_GRPC_ENDPOINT when BIGQUERY_EMULATOR_HOST is set and using gRPC, or use REST (NewRESTClient)")
			}
			grpcOpts = append(grpcOpts, v2...)
		}
		// Create clients that use gRPC as the underlying transport protocol.
		bqClient, err = apiv2_client.NewClient(ctx, grpcOpts...)
		if err != nil {
		...
		restOpts := append(append([]option.ClientOption{}, opts...), bqopts.ClientOptions()...)
		bqClient, err = apiv2_client.NewRESTClient(ctx, restOpts...)
		if err != nil {
