...
func appendToDefaultStream(w io.Writer, projectID, datasetID, tableID string) error {
	...
	clientOpts := append([]option.ClientOption{managedwriter.WithMultiplexing()}, bqopts.StorageGRPCClientOptions()...)
	client, err := managedwriter.NewClient(ctx, projectID, clientOpts...)
	if err != nil {
