...
func TestAppends(t *testing.T) {
	...
	if os.Getenv("BIGQUERY_EMULATOR_HOST") != "" && os.Getenv("BIGQUERY_STORAGE_GRPC_ENDPOINT") == "" {
		t.Skip("ManagedWriter uses BigQuery Storage gRPC; set BIGQUERY_STORAGE_GRPC_ENDPOINT to the emulator gRPC host:port or unset BIGQUERY_EMULATOR_HOST")
	}
	ctx := context.Background()

	client, err := bigquery.NewClient(ctx, tc.ProjectID, bqopts.ClientOptions()...)
	if err != nil {
		t.Fatal(err)
	}
	defer client.Close()
