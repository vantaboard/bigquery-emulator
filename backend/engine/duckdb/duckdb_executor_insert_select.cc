...
absl::StatusOr<DmlStats> RunInsertSelect(
  ...
  const schema::TableSchema& full_schema = target_table->bq_schema();
  ...
  if (::duckdb_query(conn, select_sql.c_str(), &result) != ::DuckDBSuccess) {
  ...
  absl::Status appended = storage->AppendRows(target_id, rows);
