...
absl::Status TypeMismatch(SketchType expected) {
  return MakeSemanticError(
      SemanticErrorReason::kInvalidArgument,
...
uint16_t PrecisionToK(int64_t precision) {
...
absl::StatusOr<int64_t> ParseAndValidateNumQuantiles(int64_t num_quantiles) {
...
absl::StatusOr<double> ParseAndValidatePhi(const Value& phi) {
...
absl::StatusOr<int64_t> ParseAndValidateWeight(const Value& weight) {
...
std::optional<int64_t> FirstNonNullInt64(
...
std::optional<double> FirstNonNullFloat64(
    const std::vector<std::vector<Value>>& input_column_values,
    size_t column_index) {
  if (column_index >= input_column_values.size()) return std::nullopt;
  for (const Value& v : input_column_values[column_index]) {
    if (v.is_null()) continue;
    if (v.type_kind() == ::googlesql::TYPE_DOUBLE) return v.double_value();
...
std::string SerializeSketchBytes(SketchType type,
  ...
  out.push_back(static_cast<char>(type == SketchType::kInt64 ? kSketchTypeInt64
                                                             : kSketchTypeFloat64));
  out.append(ds_payload.data(), ds_payload.size());
  return out;
}

absl::StatusOr<std::pair<SketchType, std::string>> ParseSketchEnvelope(
...
std::string SerializeDataSketchesSketch(const SketchT& sketch) {
  const auto bytes = sketch.serialize();
  return std::string(reinterpret_cast<const char*>(bytes.data()), bytes.size());
}

template <typename SketchT>
absl::StatusOr<SketchT> DeserializeDataSketchesSketch(absl::string_view payload) {
  try {
...
template <typename SketchT>
absl::StatusOr<std::string> SerializeTypedSketch(const SketchT& sketch,
                                                 SketchType type) {
  return SerializeSketchBytes(type, SerializeDataSketchesSketch(sketch));
}

template <typename SketchT>
absl::StatusOr<SketchT> DeserializeTypedSketch(absl::string_view bytes,
                                               SketchType expected) {
...
template <typename SketchT>
void UpdateWithWeight(SketchT* sketch, const typename SketchT::value_type& item,
                      int64_t weight) {
...
template <typename SketchT>
absl::StatusOr<Value> BuildQuantileArray(const SketchT& sketch,
  ...
  const ::googlesql::ArrayType* arr_type =
      return_type != nullptr && return_type->IsArray()
          ? return_type->AsArray()
          : nullptr;
  if (arr_type == nullptr) {
    return MakeSemanticError(SemanticErrorReason::kInvalidArgument,
  ...
  if constexpr (std::is_same_v<typename SketchT::value_type, int64_t>) {
    ...
    for (int64_t i = 1; i < num_quantiles; ++i) {
      const double rank = static_cast<double>(i) / static_cast<double>(num_quantiles);
...
absl::StatusOr<Value> InitAggregateImpl(
    const std::vector<std::vector<Value>>& input_column_values,
...
template <typename SketchT, SketchType TypeTag>
absl::StatusOr<SketchT> MergeSketchesImpl(
    const std::vector<std::vector<Value>>& input_column_values) {
  if (input_column_values.empty()) {
    return MakeSemanticError(SemanticErrorReason::kInvalidArgument,
...
template <typename SketchT, SketchType TypeTag>
absl::StatusOr<Value> MergePartialImpl(
    const std::vector<std::vector<Value>>& input_column_values) {
...
template <typename SketchT, SketchType TypeTag>
absl::StatusOr<Value> ExtractScalarImpl(
...
template <typename SketchT, SketchType TypeTag>
absl::StatusOr<Value> ExtractPointScalarImpl(const std::vector<Value>& args) {
  if (args.size() != 2) {
    return MakeSemanticError(SemanticErrorReason::kInvalidArgument,
...
template <typename SketchT, SketchType TypeTag>
absl::StatusOr<Value> MergeAndExtractImpl(
    const std::vector<std::vector<Value>>& input_column_values,
    const ::googlesql::Type* return_type) {
  auto merged = MergeSketchesImpl<SketchT, TypeTag>(input_column_values);
  if (!merged.ok()) {
...
template <typename SketchT, SketchType TypeTag>
absl::StatusOr<Value> MergeAndExtractPointImpl(
    const std::vector<std::vector<Value>>& input_column_values) {
  auto merged = MergeSketchesImpl<SketchT, TypeTag>(input_column_values);
  if (!merged.ok()) {
    if (merged.status().message().find("only NULL") != std::string::npos) {
...
absl::StatusOr<Value> KllQuantilesInitInt64Aggregate(
  ...
  return KllQuantilesInitInt64Values(input_column_values[0], precision, weights);
}
...
absl::StatusOr<Value> KllQuantilesInitFloat64Aggregate(
  ...
  const std::vector<Value>* weights =
      input_column_values.size() >= 3 ? &input_column_values[2] : nullptr;
  return KllQuantilesInitFloat64Values(input_column_values[0], precision,
                                       weights);
}
...
absl::StatusOr<Value> KllQuantilesInitInt64Values(
  ...
  if (weights != nullptr) {
    if (!precision.has_value()) {
      columns.push_back({Value::Int64(kDefaultPrecision)});
    }
    columns.push_back(*weights);
  }
  return InitAggregateImpl<Int64Sketch, SketchType::kInt64>(
...
absl::StatusOr<Value> KllQuantilesInitFloat64Values(
  ...
  return InitAggregateImpl<Float64Sketch, SketchType::kFloat64>(
...
absl::StatusOr<Value> KllQuantilesMergePartialAggregate(
  ...
  auto envelope = ParseSketchEnvelope(first_non_null.bytes_value());
  if (!envelope.ok()) return envelope.status();
  if (envelope->first == SketchType::kInt64) {
    return MergePartialImpl<Int64Sketch, SketchType::kInt64>(
        input_column_values);
  }
  return MergePartialImpl<Float64Sketch, SketchType::kFloat64>(
      input_column_values);
}
...
absl::StatusOr<Value> KllQuantilesMergeInt64Aggregate(
    const std::vector<std::vector<Value>>& input_column_values,
    const ::googlesql::Type* return_type) {
  return MergeAndExtractImpl<Int64Sketch, SketchType::kInt64>(
...
absl::StatusOr<Value> KllQuantilesMergeFloat64Aggregate(
    const std::vector<std::vector<Value>>& input_column_values,
    const ::googlesql::Type* return_type) {
  return MergeAndExtractImpl<Float64Sketch, SketchType::kFloat64>(
      input_column_values, return_type);
}

absl::StatusOr<Value> KllQuantilesMergePointInt64Aggregate(
    const std::vector<std::vector<Value>>& input_column_values) {
  return MergeAndExtractPointImpl<Int64Sketch, SketchType::kInt64>(
      input_column_values);
}

absl::StatusOr<Value> KllQuantilesMergePointFloat64Aggregate(
    const std::vector<std::vector<Value>>& input_column_values) {
  return MergeAndExtractPointImpl<Float64Sketch, SketchType::kFloat64>(
      input_column_values);
}
...
absl::StatusOr<Value> KllQuantilesExtractInt64Scalar(
    const std::vector<Value>& args,
    const ::googlesql::Type* return_type) {
  return ExtractScalarImpl<Int64Sketch, SketchType::kInt64>(args, return_type);
}

absl::StatusOr<Value> KllQuantilesExtractFloat64Scalar(
    const std::vector<Value>& args,
    const ::googlesql::Type* return_type) {
  return ExtractScalarImpl<Float64Sketch, SketchType::kFloat64>(args,
                                                                return_type);
}

absl::StatusOr<Value> KllQuantilesExtractPointInt64Scalar(
    const std::vector<Value>& args) {
  return ExtractPointScalarImpl<Int64Sketch, SketchType::kInt64>(args);
}

absl::StatusOr<Value> KllQuantilesExtractPointFloat64Scalar(
    const std::vector<Value>& args) {
  return ExtractPointScalarImpl<Float64Sketch, SketchType::kFloat64>(args);
}
