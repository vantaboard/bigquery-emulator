...
absl::Status ApplyAnalyticCountRange(
  ...
  for (size_t r = 0; r < out_rows.size(); ++r) {
    ...
    auto count_or = CountRowsInRangeFrame(
        r, afn, layout, input_rows, order_col_id, *low_or, has_low, *high_or,
        has_high, ctx);
...
absl::Status ApplyAnalyticFunction(
  ...
  if (fname == "count") {
    if (IsCountRangeFrame(*afn, order_spec)) {
      return ApplyAnalyticCountRange(*afn, order_spec, layout, input_rows,
                                     out_col_id, ctx, out_rows);
    }
  }
  if (fname == "sum") {
    return ApplyAnalyticSum(*afn, layout, input_rows, out_col_id, ctx,
                            out_rows);
  }
  if (fname == "percentile_cont") {
    return ApplyAnalyticPercentileCont(*afn, layout, input_rows, out_col_id,
                                       ctx, out_rows);
  }
  return MakeSemanticError(
      SemanticErrorReason::kNotImplemented,
...
absl::StatusOr<std::vector<ColumnBindings>> MaterializeAnalyticScan(
  ...
  for (int g = 0; g < analytic.function_group_list_size(); ++g) {
    ...
    for (int f = 0; f < group->analytic_function_list_size(); ++f) {
      ...
      absl::Status applied =
          ApplyAnalyticFunction(*cc, *group, order_spec, layout, input_rows,
                                ctx, out_rows);
