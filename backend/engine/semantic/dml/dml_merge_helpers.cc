...
absl::StatusOr<MergeParticipants> LoadMergeParticipants(
  ...
  MergeParticipants out;
  out.targets.reserve(target_rows_or->size());
  for (const storage::Row& row : *target_rows_or) {
    auto bind = BindRow(row, *merge.table_scan(), target_by_id, schema);
    if (!bind.ok()) return bind.status();
    out.targets.push_back({*std::move(bind), row, false, 0, {}, false, false});
  }
  out.sources.reserve(source_rows_or->size());
  for (const ColumnBindings& bind : *source_rows_or) {
    out.sources.push_back({bind, false, false});
  }
  return out;
}
    ...
    switch (when->match_type()) {
      ...
      case ::googlesql::ResolvedMergeWhen::NOT_MATCHED_BY_TARGET: {
        absl::Status applied = ApplyNotMatchedByTargetMergeWhen(
            *when, schema, participants.sources, inserts, stats, ctx);
        if (!applied.ok()) return applied;
        break;
      }
