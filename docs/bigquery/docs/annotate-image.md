...
To create a remote model and annotate images, you need the
...
`ML.ANNOTATE_IMAGE` function. If you want to call the
`ML.ANNOTATE_IMAGE` function in a different project than the one
...
The following example detects any faces shown in the images, and also returns
image attributes, like dominant colors:
