...
| Curl Request |
|---|
| ```bash curl --location 'https://bigquerydatatransfer.googleapis.com/mcp' \ --header 'content-type: application/json' \ --header 'accept: application/json, text/event-stream' \ --data '{ "method": "tools/call", "params": { "name": "list_transfer_configs", "arguments": { // provide these details according to the tool's MCP specification } }, "jsonrpc": "2.0", "id": 1 }' ``` |
...
| JSON representation |
|---|
| ``` { "parent": string, "dataSourceIds": [ string ], "pageToken": string, "pageSize": integer } ``` |
...
| JSON representation |
|---|
| ``` { "transferConfigs": [ { object (`https://docs.cloud.google.com/bigquery/docs/reference/datatransfer/mcp/tools_list/create_transfer_config#Output.Schema.TransferConfig`) } ], "nextPageToken": string } ``` |

| Fields ||
|---|---|
| `transferConfigs[]` | ``object (`https://docs.cloud.google.com/bigquery/docs/reference/datatransfer/mcp/tools_list/create_transfer_config#Output.Schema.TransferConfig`)`` Output only. The stored pipeline transfer configurations. |
