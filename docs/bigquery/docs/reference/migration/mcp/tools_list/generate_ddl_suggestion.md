...
| Curl Request |
|---|
| ```bash curl --location 'https://bigquerymigration.googleapis.com/mcp' \ --header 'content-type: application/json' \ --header 'accept: application/json, text/event-stream' \ --data '{ "method": "tools/call", "params": { "name": "generate_ddl_suggestion", "arguments": { // provide these details according to the tool's MCP specification } }, "jsonrpc": "2.0", "id": 1 }' ``` |
...
| JSON representation |
|---|
| ``` { "parent": string, "inputQuery": string, "sourceDialect": string } ``` |

| Fields ||
|---|---|
| `parent` | `string` Required. The parent resource name where the DDL suggestion is generated. Format: `projects/PROJECT_ID/locations/LOCATION`. |
| `inputQuery` | `string` Required. The query string for which the DDL suggestion is generated. |
...
| JSON representation |
|---|
| ``` { "suggestionId": string, "suggestionState": string, "logs": [ { object (`https://docs.cloud.google.com/bigquery/docs/reference/migration/mcp/tools_list/translate_query#Output.Schema.Log`) } ], "errorInfo": { object (`https://docs.cloud.google.com/bigquery/docs/reference/migration/mcp/tools_list/translate_query#Output.Schema.ErrorInfo`) } } ``` |

| Fields ||
|---|---|
| `suggestionId` | `string` The suggestion ID of the DDL suggestion. Use this ID to fetch the DDL suggestion. Format: `projects/PROJECT_ID/locations/LOCATION/suggestions/SUGGESTION_ID`. |
| `suggestionState` | `string` The current state of the DDL suggestion, for example, `RUNNING`, `SUCCEEDED` or `FAILED`. |
