...
| KLL (`KLL_QUANTILES.*`)                                                                                      | `unsupported` (planned `semantic_executor`) | Real BigQuery approximate-quantile sketches (bq-verified). Registered `semantic_executor status=planned` in `functions.yaml`; surfaces `UNIMPLEMENTED` until the local handler lands paralleling `HLL_COUNT.*`. See ROADMAP §"KLL quantile sketches". |
