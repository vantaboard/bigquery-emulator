...
| Protobuf shapes (`ResolvedMakeProto`, `ResolvedGetProtoField`, ...)                                          | `local_impl`  | Routes to the semantic executor (`eval_expr_proto.cc`, `proto_helpers.cc`). PROTO values persist as serialized BYTES; descriptor pool comes from the analyzer/catalog `SimpleCatalog::SetDescriptorPool`. Construct/read (`MakeProto`, `GetProtoField`, `GetProtoOneof`), mutate (`ReplaceField`, `FilterField`), and row field access (`GetRowField`) evaluate locally. Pinned by `eval_expr_proto_test`; STRUCT `REPLACE_FIELDS` also in `conformance/fixtures/specialized/replace_fields_struct.yaml`. Proto-typed CREATE TABLE / googlesql-corpus `load_proto_*` cases remain deferred until catalog proto registration lands. |
