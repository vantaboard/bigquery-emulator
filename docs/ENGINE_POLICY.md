...
| Scripting (`DECLARE`, `SET`, `CALL`, `BEGIN…END` multi-stmt blocks)                                          | `semantic_executor`     | Gateway sends DECLARE/SET/CALL scripts in one engine `ExecuteQuery` round-trip (`script_runner_engine.go`). Simple statements (`CREATE CONSTANT`/`SET`/`CALL`/`ASSERT`/`EXECUTE IMMEDIATE` literals) run via `ExecuteScriptViaAnalyzeNext`; scripts with structured control flow (`IF`/`WHILE`/`LOOP`/`FOR`/`EXECUTE IMMEDIATE` in blocks) delegate to `googlesql::ScriptExecutor` through `EmulatorStatementEvaluator` when `BIGQUERY_EMULATOR_HAS_GOOGLESQL_SCRIPTING=1` (`GOOGLESQL_SOURCE=local`). Prebuilt artifacts without scripting still surface `UNIMPLEMENTED` for control flow. `BEGIN…EXCEPTION` / `@@error.*` / `RAISE` remain open. |
